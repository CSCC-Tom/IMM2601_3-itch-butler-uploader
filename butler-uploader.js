...
function CurrentVersionString() {
    ...
    const fullHours = Math.floor(msLeft / msPerHour).toString(10).padStart(2, '0');
    msLeft -= fullHours * msPerHour;
    const fullMinutes = Math.floor(msLeft / msPerMinute).toString(10).padStart(2, '0');
...
function UploadTeamGame(platform, version_string, cb) {
    ...
    const butler_command_args = ['push', platform, `${username}/${gamename}:${platform}`, `--userversion`, version_string];
    ...
    execFile(exePath, butler_command_args,
        (error, stdout, stderr) => {
        if (error) {
...
function RunButlerUploader() {
    ...
    console.log(`project start = ${proj_date.toDateString()} @ ${proj_date.toTimeString()}`)
    console.log('So, version number is ' + version_string);
    const args = process.argv;
    if(args.length > 2) {
        console.log('No Windows-32 build being supplied. Poor forsaken gamers...\n\n')
        ButlerUploadWin(version_string);
    } else {
        console.log('You are awesome for providing a 32-bit Windows binary for deprived gamers!\n\n')
